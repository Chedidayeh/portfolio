...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-2xl  px-4 pb-3 lg:flex bg-neutral-900",
        className,
      )}
    >
      {items.map((item) => (
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute -top-8 left-1/2 w-fit rounded-md border font-semibold  px-2 py-0.5 text-xs whitespace-pre border-neutral-900 bg-neutral-800 text-white"
            >
              {title}
            </motion.div>
          )}
